...
import { getFarmerOrders, confirmOrder, cancelOrder, type OrderResponse } from "~/services/orders";
import { getRetailerProducts } from "~/services/products";
import { addNotification } from "~/services/notifications";

export function meta() {
  return [
    { title: "Orders Received | SmartX AgriTrade" },
    { name: "description", content: "View orders received by the farmer." },
  ];
}
...
export default function FarmerOrdersPage() {
  ...
  async function loadOrders() {
    ...
    try {
      const resp = await getFarmerOrders();
      ...
      const farmerMockOrders = mockOrders.filter((o: any) => o.farmerId === Number(user?.id));
      
      // Combine and Sort based on New Request: 
      // 1. Status Priority (CONFIRMED > PENDING > CANCELLED)
      // 2. Date Descending (Newest first)
      const combined = [...farmerMockOrders, ...apiOrders];
  ...
  async function handleConfirm(orderId: number) {
    ...
    try {
      ...
      if (orderData) {
        // Try to get product name from items
        const firstItem = orderData.items?.[0];
        const prodName = productNames[firstItem?.productId] || (firstItem as any)?.productName || "your requested products";
        
        await addNotification(
          orderData.retailerId,
          "Order Accepted!",
          `Farmer ${user?.name || "A farmer"} has accepted your order for "${prodName}".`,
          "ORDER"
        );
      }
      ...
      if (isMockOrder || err.response?.status === 403 || err.response?.status === 404) {
        console.warn(`Mocking confirm success for order ${orderId}.`);
        ...
        // Alert Retailer about confirmation (MOCK path)
        const orderData = orders.find(o => o.id === orderId);
        if (orderData) {
          const firstItem = orderData.items?.[0];
          const prodName = productNames[firstItem?.productId] || (firstItem as any)?.productName || "your products";
          await addNotification(
            orderData.retailerId,
            "Order Accepted!",
            `Farmer ${user?.name || "A farmer"} has accepted your order for "${prodName}".`,
  ...
  async function handleCancel(orderId: number) {
    ...
    try {
      ...
      if (orderData) {
        // Try to get product name from items
        const firstItem = orderData.items?.[0];
        const prodName = productNames[firstItem?.productId] || (firstItem as any)?.productName || "your order";
        
        await addNotification(
          orderData.retailerId,
          "Order Cancelled",
          `Farmer ${user?.name || "A farmer"} has cancelled your order for "${prodName}".`,
          "ORDER"
        );
      }
      ...
      if (isMockOrder || err.response?.status === 403 || err.response?.status === 404) {
        ...
        // Alert Retailer about cancellation (MOCK path)
        const orderData = orders.find(o => o.id === orderId);
        if (orderData) {
          const firstItem = orderData.items?.[0];
          const prodName = productNames[firstItem?.productId] || (firstItem as any)?.productName || "your order";
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Orders Received</h1>
            <p className="mt-1 text-gray-500 dark:text-gray-400">View and manage orders placed by retailers.</p>
          </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                      ...
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusBadgeClass(order.status)}`}>
                        {order.status}
                      </span>
                    </div>
                  ...
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                    <button
                      ...
                      disabled={confirmingId === order.id || order.status !== "PENDING"}
                    ...
                    >
                      {confirmingId === order.id ? "Confirming..." : "Accept"}
                    </button>
                    <button
